...
- **Simplicity over configurability** — favor sensible defaults, remove optional parameters, be opinionated rather than flexible.
- **Never lower the bar.** Never weaken thresholds, disable checks, relax lint rules, or skip tests to make something pass. Fix the underlying code.
- **Never estimate effort or duration for human work.** Focus on what needs to be done.
...
- `Idea` — the core entity with fields: `nombre`, `idea`, `descripcion`, `plan_de_negocio`, `estructura`, `planificacion`
- `costo` — cost level: Bajo / Medio / Alto
- `prioridad` — priority: Alta / Media / Baja
- `estado` — status: Idea / En analisis / En desarrollo / Descartada
